...
for exp = 1:N_REPLIC %Replicaciones
    ...
    for i=1:muestras
        ...
        Ry_gt(:,:,i) = [cos(pitch_gt(i)) 0 sin(pitch_gt(i)); 0 1 0; -sin(pitch_gt(i)) 0 cos(pitch_gt(i))];
        Rz_gt(:,:,i) = [cos(yaw_gt(i)) -sin(yaw_gt(i)) 0; sin(yaw_gt(i)) cos(yaw_gt(i)) 0; 0 0 1];
        R_gt(:,:,i) = Rx_gt(:,:,i)*Ry_gt(:,:,i)*Rz_gt(:,:,i);
        % Matrices Rot Loc
        Rx_loc(:,:,i) = [1 0 0; 0 cosd(roll_loc(i)) -sind(roll_loc(i)); 0 sind(roll_loc(i)) cosd(roll_loc(i))];
        Ry_loc(:,:,i) = [cos(pitch_loc(i)) 0 sin(pitch_loc(i)); 0 1 0; -sin(pitch_loc(i)) 0 cos(pitch_loc(i))];
        Rz_loc(:,:,i) = [cos(yaw_loc(i)) -sin(yaw_loc(i)) 0; sin(yaw_loc(i)) cos(yaw_loc(i)) 0; 0 0 1];

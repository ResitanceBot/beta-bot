...
public:
  void Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) override {
    ...
    ROS_INFO("ReceiverModelPlugin: receiver_name: %s",
             this->receiver_name.c_str());
    ROS_INFO("ReceiverModelPlugin: send_sim_pose: %d", this->send_sim_pose);
    ROS_INFO("ReceiverModelPlugin: use_sync: %d", this->use_sync);
    ROS_INFO("ReceiverModelPlugin: packed_publishing: %d",
             this->packed_publishing);

    ros::NodeHandle n;

    if (this->send_sim_pose)
      this->sim_pose_pub = n.advertise<geometry_msgs::PoseStamped>(
          "/beacons_gazebo/" + this->receiver_name + "/pose", 10);
    if (!this->packed_publishing)
      this->receiver_in_msgs_publisher =
          n.advertise<beacons_gazebo::ReceiverIn>(
              "/" + this->model->GetName() + "/" + this->receiver_name +
                  "/receiver_in_msgs",
              10);
    else
      this->receiver_in_msgs_packed_publisher =
          n.advertise<beacons_gazebo::ReceiverInSyncPacked>(
              "/" + this->model->GetName() + "/" + this->receiver_name +
                  "/receiver_in_msgs_packed",
              10);
    ...
    if (!this->use_sync) {
      ...
      this->sync_sub =
          this->gz_node->Subscribe(std::string("/beacon_sim_gazebo/sync"),
                                   &ReceiverModelPlugin::OnSync, this);
    }
  }
...
private:
  void UpdateFun(const common::Time &simTime) {
    if (this->send_sim_pose)
      this->sendSimPose();
    ...
    for (auto &model : models) {
      ...
      for (auto &link : model_links) {
        if (link->GetName().find("beacon") == 0) {
          ...
          if (this->rssi_noise_generators.find(beacon_name) ==
              this->rssi_noise_generators.end()) {
            this->rssi_noise_generators[beacon_name] =
                beacons_gazebo::RSSINoise(this->m_rssi);
          }

          auto beacon_p = link->WorldPose().Pos();
          auto receiver_p = this->receiver_link->WorldPose().Pos();

          double d = std::sqrt(std::pow(beacon_p.X() - receiver_p.X(), 2) +
          ...
          msg.rssi = this->rssi_noise_generators[beacon_name].getRSSI(
              d, simTime.Double());
          msg.id = beacon_name;
